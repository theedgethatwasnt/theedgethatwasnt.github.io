...
SOP compliance (CLAUDE.md §Backtest-Live Consistency SOP):
  ...
  R6  TRState + process_bar() logic identical to paper service and backtest kernel

Output: /data/logs/tr_live_status.json
        /data/logs/tr_live_trades.jsonl
"""
...
logging.basicConfig(
    ...
    format="%(asctime)s %(levelname)s [tr_live] %(message)s",
)
logger = logging.getLogger("tr_live")

STATE_DIR = os.environ.get("TR_LIVE_STATE_DIR", "/tmp")
UNITS     = int(os.environ.get("TR_LIVE_UNITS", "10"))
...
class TRLiveSim:
    """
    Runs one TR-momentum config on live M5 bars and places real OANDA orders.
    R6: process_bar() logic identical to paper service TRPaperSim and backtest kernel.
    """

    def __init__(self, cfg: TRConfig, adapter: OANDAAdapter):
        self.cfg              = cfg
        self.adapter          = adapter
        self.st               = TRState()
        self.oanda_trade_id:  Optional[str] = None
    ...
    def process_bar(self, bar: dict, sp: float, ts: str) -> Optional[TradeRecord]:
        ...
        if st.pos != 0:
            if st.pos == 1:
                ...
                trail = st.hw - trail_p
                if l <= trail:
                    # Place close order — use real fill price for PnL
                    if self.oanda_trade_id is not None:
                        broker_result = self.adapter.close_trade(self.oanda_trade_id)
                        fill_px = broker_result.fill_price if broker_result.success else trail
                        if not broker_result.success:
                            logger.error(
                                f"[{cfg.label}] Close trade failed: "
                                f"{broker_result.error} — using trail level"
                            )
                    else:
                        fill_px = trail
                        logger.warning(f"[{cfg.label}] Exit triggered but no oanda_trade_id")

                    pnl_pips = (fill_px - st.entry_px) / pip - sp
                    new_rec  = self._close(pnl_pips, fill_px, sp, ts, "trail")
                ...
                if h >= trail:
                    ...
                    pnl_pips = (st.entry_px - fill_px) / pip - sp
                    new_rec  = self._close(pnl_pips, fill_px, sp, ts, "trail")
                    self.oanda_trade_id = None

        # ── ENTRY ─────────────────────────────────────────────────────────────
        # NO_NEW_ENTRIES kill switch: manage existing positions but skip new entries.
        if os.environ.get('NO_NEW_ENTRIES', '') == '1' and st.pos == 0:
            return new_rec
        if st.pos == 0 and tr_price >= cfg.tr_thresh * pip and sp <= cfg.sp_gate:
            ...
            broker_result = self.adapter.place_market_order(cfg.pair, oanda_units)

            if broker_result.success:
                self.oanda_trade_id = broker_result.trade_id
                fill_px             = broker_result.fill_price
                st.pos              = direction
                st.entry_px         = fill_px      # use actual fill price
                ...
                logger.info(
                    f"[{cfg.label}] ENTRY {dir_str} @ {fill_px:.5f} "
                    f"tid={self.oanda_trade_id} TR={self.tr_at_entry:.1f}p"
                )
                _tg(
                    f"{'🟢' if direction==1 else '🔴'} TR LIVE {cfg.label} "
                    f"{dir_str} {cfg.pair} @{fill_px:.5f} {UNITS}u"
                )
            elif broker_result.error == "WEEKEND_ENTRY_BLOCKED":
                logger.info(f"[{cfg.label}] Weekend block — entry skipped")
            else:
                logger.error(
                    f"[{cfg.label}] Entry order failed: {broker_result.error} — staying flat"
                )
                # Do NOT update state — order didn't go through
    ...
    def _close(self, pnl_pips: float, exit_px: float, sp: float,
        ...
        write_trade_direct(
            strategy    = "tr_live",
            pair        = cfg.pair,
            account_id  = os.environ.get("OANDA_ACCOUNT_TR_LIVE", ""),
            trade_id    = f"tr_live_{cfg.label}_{self.entry_time}",
            ...
            units       = UNITS,
            mfe_pips    = 0.0, mae_pips=0.0, capture_ratio=0.0,
            is_paper    = False, label=cfg.label,
        )
        return rec
...
# ── Status writer ─────────────────────────────────────────────────────────────
def write_status(sims: List[TRLiveSim], path: str) -> None:
    data = {
...
async def run_live(shutdown_event: asyncio.Event) -> None:
    acct_id  = os.environ.get("OANDA_ACCOUNT_TR_LIVE",
    ...
    sims     = [TRLiveSim(cfg, adapter) for cfg in CONFIGS]
    pair_sims: Dict[str, List[TRLiveSim]] = {p: [] for p in PAIRS_NEEDED}
    for s in sims:
        pair_sims[s.cfg.pair].append(s)

    status_path = os.path.join(STATE_DIR, "tr_live_status.json")
    trades_path = os.path.join(STATE_DIR, "tr_live_trades.jsonl")

    logger.info(
        f"TR Momentum live trading started — {len(sims)} configs on {acct_id} "
        f"({UNITS}u/trade)"
    )
    for s in sims:
        logger.info(
            f"  [{s.cfg.label:12s}] TR≥{s.cfg.tr_thresh:.0f}p trail={s.cfg.trail_pips:.0f}p "
            f"sp_gate={s.cfg.sp_gate}p  OOS_ref={s.cfg.oos_pd}p/d"
        )

    # ── Warmup: prime prev_close and re-adopt any existing open positions ─────
    logger.info("Warming up — fetching last M5 bar per pair…")
    loop = asyncio.get_event_loop()
    for pair, fetcher in fetchers.items():
        try:
            candles = await loop.run_in_executor(
                None, lambda p=pair: adapter.get_candles(p, count=2, granularity="M5")
            )
            if candles:
                last = candles[-1]
                fetcher.last_ts = last['timestamp']
                for sim in pair_sims[pair]:
                    sim.st.prev_close = float(last['close'])
                    sim.st.initialized = True
                    sim.bar_count = 1
                logger.info(f"  {pair}: prev_close={float(last['close']):.5f} ts={last['timestamp']}")
        except Exception as exc:
            logger.warning(f"  {pair}: warmup fetch failed: {exc}")

    logger.info("Checking for existing open positions to re-adopt…")
    try:
        open_trades = await asyncio.wait_for(
            loop.run_in_executor(None, adapter.get_open_trades),
            timeout=30.0,
        )
        for trade in open_trades:
            instrument = trade.instrument
            if instrument not in pair_sims:
                continue
            direction = 1 if trade.units > 0 else -1
            for sim in pair_sims[instrument]:
                if sim.st.pos == 0:
                    sim.st.pos = direction
                    sim.st.entry_px = trade.entry_price
                    sim.st.hw = trade.entry_price
                    sim.oanda_trade_id = trade.trade_id
                    sim.entry_time = trade.open_time or ""
                    sim.entry_bar = sim.bar_count
                    logger.info(
                        f"  [{sim.cfg.label}] Re-adopted "
                        f"{'LONG' if direction==1 else 'SHORT'} @ {trade.entry_price:.5f} "
                        f"tid={trade.trade_id}"
                    )
                    break
    except asyncio.TimeoutError:
        logger.warning("get_open_trades timed out after 30s — skipping re-adoption")
    except Exception as exc:
        logger.warning(f"Open trade re-adoption failed: {exc}")

    _tg(
        f"🚀 TR LIVE started — {len(sims)} configs on 011\n"
        f"{UNITS}u/trade | Pairs: {', '.join(PAIRS_NEEDED)}"
    )
    ...
    while not shutdown_event.is_set():
        ...
        for pair, fetcher in fetchers.items():
            try:
                ...
                hl = (float(bar['high']) - float(bar['low'])) / _pip
                logger.info(f"[{pair}] bar {ts} cl={bar['close']} hl={hl:.1f}p sp={sp:.2f}p")
                ...
                for sim in pair_sims[pair]:
                    ...
                    if rec is not None:
                        ...
                        _tg(
                            f"{emoji} TR LIVE {rec.label}\n"
                            f"011 {dir_str} {rec.exit_reason}: {rec.pnl_pips:+.2f}p "
        ...
        if any_new:
            ...
            if sims[0].bar_count > 1 and sims[0].bar_count % 288 == 0:
                ...
                lines = [f"📊 TR Live Day {days:.0f} summary (011):"]
                for s in sims:
