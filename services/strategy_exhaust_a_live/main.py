#!/usr/bin/env python3
"""
Exhaustion Continuation A Live — Account 003, 12 pairs
=======================================================
...
Validation: n_consec=4, dist_mult=2.0, TP=15p — 11/12 pairs IS 3/3 + MC p<0.05
  Portfolio mc_p=0.0080  portf_pd=+32.4 p/d
...
logging.basicConfig(
    ...
    format="%(asctime)s %(levelname)s [exhaust_a] %(message)s",
)
logger = logging.getLogger("exhaust_a")

ACCOUNT_ID       = os.environ.get("OANDA_ACCOUNT_ID_003", "<OANDA_ACCOUNT_ID>")
STATE_DIR        = os.environ.get("EXHAUST_A_STATE_DIR", "/data/logs")
UNITS_PER_DOLLAR = 1.25
_units           = 25
_last_balance_refresh = 0.0
STRATEGY  = "exhaust_cont_a"
LABEL     = "exhaust_a_003"
...
def _state_path(pair: str) -> str:
    return os.path.join(STATE_DIR, f"exhaust_a_{pair}_state.json")
...
def _enter(st: PairState, direction: int, spread: float, adapter):
    ...
    _tg(f"{dir_sym} 003 {st.pair} OPEN {dir_str} @ {fill_px:.3f}  {entry_units}u\n"
...
def _record_close(st: PairState, exit_px: float, exit_reason: str):
    ...
    _tg(f"{pl_sym} 003 {st.pair} CLOSE {dir_sym}  {exit_reason}\nP/L {pnl_pips:+.1f}p  held {dur_h:.1f}h")
...
def main():
    ...
    if not ACCOUNT_ID:
        logger.error("OANDA_ACCOUNT_ID_003 not set — exiting"); sys.exit(1)
    ...
    _tg(f"🟢 003 Exhaust-Cont A started\nn_consec={N_CONSEC} dist_mult={DIST_MULT} TP={TP_PIPS}p  {_units}u/trade\n"
        f"Pairs ({len(PAIRS)}): {', '.join(PAIRS)}")
    logger.info(f"Exhaust-Cont A live — acct={ACCOUNT_ID}  {len(PAIRS)} pairs  {_units}u")
    while not _shutdown:
        if not _market_open():
