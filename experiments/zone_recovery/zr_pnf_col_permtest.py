"""
Permutation test + Bootstrap MC for top COL-direction P&F configs
on non-AUD_JPY pairs (AUD_JPY COL already tested in zr_pnf_permtest.py).

Candidates: NZD_JPY(5,4), NZD_JPY(10,3), NZD_JPY(10,4),
            CHF_JPY(5,3), CHF_JPY(5,2), EUR_JPY(5,4), GBP_USD(5,2)
All have WF=3 in sweep. Appends results to zr_pnf_permtest_results.csv.
"""
import math, sys
import numpy as np
import pandas as pd
...
# COL configs to test: (pair, box_pips, reversal, dir_mode=1, label)
CONFIGS = [
    ("NZD_JPY",  5, 4, 1, "col"),
    ("NZD_JPY", 10, 3, 1, "col"),
    ("NZD_JPY", 10, 4, 1, "col"),
...
# ─── ZR sim returning per-cycle P&L (for bootstrap) ──────────────────────────
...
# ─── ZR sim for permutation (lightweight, alternating direction) ──────────────
...
sim_zr_pnf_cycles(_o,_h,_l,_c,_rb,_rd,1,0.0001,SPREAD,PF,MAX_LEGS,20.,10.,5.,3.)
...
for cfg in CONFIGS:
    ...
    print(f"  Gates: perm={'✅' if gate_perm else '❌'}  p5>0={'✅' if gate_p5 else '❌'}  "
          f"P(+)>95%={'✅' if gate_prob else '❌'}  → {gates_passed}/3")
...
print("\n=== COL CONFIGS SUMMARY ===")
col_only = df_new.copy()
print(f"{'Config':35} {'obs_ppd':>8} {'p-val':>7} {'p5':>7} {'p50':>7} {'p95':>7} {'P(+)':>6} {'gates':>5}")
print("─"*85)
for _, r in col_only.sort_values('obs_ppd', ascending=False).iterrows():
...
print("\n=== FULL PERMTEST — ALL RESULTS (sorted by obs_ppd) ===")
print(f"{'Config':35} {'obs_ppd':>8} {'p-val':>7} {'p5':>7} {'p50':>7} {'P(+)':>6} {'gates':>5}")
print("─"*85)
for _, r in df_all.sort_values('obs_ppd', ascending=False).iterrows():
    tag = f"{r.pair} b={r.box_pips} rv={r.reversal} {r.direction}"
    pf_flag = "✅" if r.p_value < 0.05 else "❌"
    print(f"{tag:35} {r.obs_ppd:>8.0f} {r.p_value:>7.4f}{pf_flag} "
          f"{r.boot_p5:>7.0f} {r.boot_median:>7.0f} "
          f"{r.prob_pos:>6.3f} {r.gates:>5}/3")

# ─── Append: new configs from extended rev sweep ──────────────────────────────
# Run separately: CHF_JPY b15r1.5 ALT, NZD_JPY b15r2.5 ALT, EUR_JPY b5r1.5 ALT/COL
