"""
Price-based reversal sweep: tests whether px <= pnf_level - rev*bs
outperforms the integer delta <= -rev check used in production.

Key: price-based r=1.0 at b=5 ≈ integer r=2 in strictness.
     price-based r=0.5 at b=5 ≈ integer r=1 in strictness.
     This sweep explores fractional rev values (0.5-3.0) across box 5-30.

Benchmarks (integer reversal, for comparison):
  GBP_JPY b5_r1_n4_E2_X3c_1_5 = 71.6 p/d  (ft, failed live)
  USD_JPY b5_r1_n3_E2_X3c_1_5 = 68.5 p/d  (ft, failed live)
  EUR_JPY b5_r1_n3_E2_X3c_1_5 = 39.9 p/d  (ft)
  GBP_USD b2_r3_n8_E2_X3c_1_5 = 54.5 p/d  (b2, in paper)

Run:
    cd /path/to/projects/fx-core
    python3 research/experiments/fifo_trends/backtest_price_based_rev.py
"""
...
# ── Parameter grid ──────────────────────────────────────────────────────────
BOX_SIZES   = [5.0, 10.0, 15.0, 20.0, 30.0]       # pips — matches original sweep
REVERSALS   = [0.5, 1.0, 1.5, 2.0, 2.5, 3.0]      # fractional boxes
TRAIL_DISTS = [1, 2, 3, 4]                          # trail in boxes
SMA_KS      = [0, 3, 5, 7]                          # col-SMA k (0=trail only)
...
IS_FRAC    = 0.70
MAX_K      = 10
MAX_TRADES = 25000   # b=5 generates many trades; safe ceiling

# Integer benchmarks for context
BENCHMARKS = {
    "GBP_JPY": 71.6,
    "USD_JPY": 68.5,
    "EUR_JPY": 39.9,
...
def config_name(row):
    ...
    trail = f"Xpb_{int(td)}"   # "pb" = price-based to distinguish from integer
...
def run_kernel(opens, highs, lows, closes, spreads, bar_chunks,
    ...
    for ci in prange(N_CONFIGS):
        ...
        rev     = configs[ci, 1]   # fractional — price-based check
        ...
        bs       = bs_pips * pip
        rev_dist = rev * bs        # reversal distance in actual price units
        trail_px = trail_d * bs    # trail stop distance in price units
        ...
        for i in range(N_BARS):
            ...
            for tick in range(2):
                ...
                if pnf_dir == 1:
                    if delta >= 1:                        # extension up (integer)
                        pnf_idx   = new_idx
                        pnf_level = pnf_idx * bs
                        col_count += delta
                    elif px <= pnf_level - rev_dist:      # reversal down (price-based)
                        prev_col = col_count
                    ...
                    if delta <= -1:                       # extension down (integer)
                        pnf_idx   = new_idx
                        pnf_level = pnf_idx * bs
                        col_count += (-delta)
                    elif px >= pnf_level + rev_dist:      # reversal up (price-based)
                        prev_col = col_count
                        col_hist[col_hist_ptr % MAX_K] = prev_col
                        col_hist_ptr += 1
                        if col_hist_n < MAX_K: col_hist_n += 1
...
def run_pair(pair, pip):
    ...
    if not ba_path.exists():
        print(f"  ⚠ No BA data: {ba_path}"); return None
...
# ── Main ─────────────────────────────────────────────────────────────────────
print("Price-based reversal sweep (px <= pnf_level - rev*bs)")
...
print("Integer-reversal benchmarks: GBP_JPY=71.6  USD_JPY=68.5  EUR_JPY=39.9  GBP_USD=54.5")
print("="*75)
...
_cfg  = np.array([[5.0, 1.0, 1.0, 5.0, 4.0, 1.0]], np.float64)
...
for pair, pip in PAIRS:
    bench = BENCHMARKS.get(pair, 0)
    print(f"\n{'─'*75}")
    print(f"  {pair}  (benchmark integer-rev: {bench} p/d)")
    ...
    if not winners:
        print("  ❌ No OOS winners"); tg(f"🔴 PB-rev {pair}: 0 winners"); continue

    best = winners[0]
    vs = "🟢 BEATS" if best['oos_pd'] > bench else "🔴 LOSES TO"
    print(f"  {vs} benchmark  ({best['oos_pd']:.1f} vs {bench})")

    print(f"\n  {'Rank':<5} {'Config':<42} {'IS p/d':>7} {'OOS p/d':>8} "
          f"{'Trades':>7} {'WR%':>5} {'AvgW':>6} {'AvgL':>7}")
    print(f"  {'─'*97}")
    for rank, w in enumerate(winners[:TOP_N], 1):
        marker = " ◀" if w['oos_pd'] > bench else ""
        print(f"  {rank:<5} {w['name']:<42} {w['is_pd']:>7.1f} {w['oos_pd']:>8.1f} "
              f"{w['oos_ntrd']:>7} {w['oos_wr']:>5.1f} {w['avg_win']:>6.1f} {w['avg_loss']:>7.1f}{marker}")

    msg = (f"{'🟢' if best['oos_pd']>bench else '🔴'} PB-rev {pair}\n"
           f"Winners: {len(winners)}/{N_CONFIGS}\n"
           f"Best: {best['name']}\n"
           f"IS={best['is_pd']:+.1f}  OOS={best['oos_pd']:+.1f} p/d\n"
           f"vs benchmark: {bench} p/d  "
           f"({'BEATS' if best['oos_pd']>bench else 'loses'})\n"
