#!/usr/bin/env python3
"""
Activation Function Experiment v2 — Expanded Wavelet Suite
============================================================
Activations tested (13 total, grouped by concept):
...
def plot_activations(close, mc_d, mc_dd, er_norm, pip, out_dir):
    ...
    out = Path(out_dir) / 'activation_shapes_v2.png'
...
def build_pop(config, n):
    pop = {}
    for i in range(n):
        g = build_genome(config, i); g.key = i; pop[i] = g
    return pop
...
def run_evolution(close, ind_arr, pip, generations=40, pop_size=40,
                  n_islands=2, migrate_every=8):
    config   = make_config()
    islands  = [build_pop(config, pop_size) for _ in range(n_islands)]
...
def plot_results(close, ind_arr, pip, best_genome, config, history, out_dir):
    ...
    fig.suptitle('NEAT v2 — 13 activations (5→5→7→3)  |  tanh/sin/cos/gauss/mex_hat/'
                 'morlet_re/morlet_im/gabor/dog/sinc/chirp/sech/haar',
    ...
    plt.tight_layout()
    out = Path(out_dir) / 'training_results_v2.png'
    plt.savefig(out, dpi=130, bbox_inches='tight')
    print(f'  Saved: {out}')
    plt.close(fig)
    ...
    plt.tight_layout()
    out2 = Path(out_dir) / 'activation_usage_v2.png'
...
def main():
    ...
    best_genome, config, history = run_evolution(
        close, ind_arr, pip,
        generations=40, pop_size=40, n_islands=2, migrate_every=8
    )
    ...
    import pickle
    pkl = OUT_DIR / 'best_genome_v2.pkl'
