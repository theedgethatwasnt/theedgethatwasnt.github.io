#!/usr/bin/env python3
"""
Activation Function Experiment v4 — 4 Clean Inputs, Seeded, 200 Gens
=====================================================================
Fixes v3: removed mex_hat(MC_D) as input — that was pre-applying an activation
before the network, defeating the purpose of testing activations.

4 clean indicator inputs:
  -1  MC(D)    — momentum direction
  -2  MC(dD)   — momentum change (derivative)
  -3  ER_norm  — trend efficiency (Kaufman, arctan-normalised)
  -4  UPnL     — unrealised P&L of current position (live, normalised)

Seeding: one pure-activation genome per activation type in gen 1.
200 generations to explore combinations via crossover.
...
Inputs: MC_D, MC_dD, ER_norm, UPnL
"""
...
def plot_activations(close, mc_d, mc_dd, er_norm, pip, out_dir):
    ...
    # ── Row 1: 4 clean indicators (spanning cols 0-4) ─────────────────────────
    ...
    indicators = [
        ...
        (np.zeros(len(close)), 'UPnL [input 4]\n(live — zero at rest)', '#95a5a6'),
    ]
    for col, (sig, label, color) in enumerate(indicators):
    ...
    # 5th column: correlation between MC(D) and ER_norm as context
    ax = fig.add_subplot(gs[1, 4])
    ax.scatter(mc_d_n[:300], er_norm[:300], s=2, color='#9b59b6', alpha=0.4)
    ax.set_title('MC(D) vs ER_norm\n(input correlation)', fontsize=8)
    ax.set_xlabel('MC(D)', fontsize=7); ax.set_ylabel('ER', fontsize=7)
    ...
    out = Path(out_dir) / 'activation_shapes_v4.png'
...
def simulate(genome, config, close, ind_arr, pip):
    ...
    for i in range(n):
        ...
        inp = [float(ind_arr[i,0]), float(ind_arr[i,1]), float(ind_arr[i,2]), upnl]
...
def plot_results(close, ind_arr, pip, best_genome, config, history, out_dir):
    ...
    fig.suptitle('NEAT v4 — 4 clean inputs, 17 activations, seeded pop, 200 gens (4→5→7→3)',
    ...
    plt.tight_layout()
    out = Path(out_dir) / 'training_results_v4.png'
    plt.savefig(out, dpi=130, bbox_inches='tight')
    print(f'  Saved: {out}')
    plt.close(fig)
    ...
    plt.tight_layout()
    out2 = Path(out_dir) / 'activation_usage_v4.png'
...
def main():
    ...
    mc_d_n       = normalise(mc_d)
    mc_dd_n      = normalise(mc_dd)
    ind_arr      = np.column_stack([mc_d_n, mc_dd_n, er_norm]).astype(np.float32)
    # UPnL is input 4 but filled live in simulate() — not pre-computed
    ...
    import pickle
    pkl = OUT_DIR / 'best_genome_v4.pkl'
