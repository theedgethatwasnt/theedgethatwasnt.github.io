#!/usr/bin/env python3
"""
CMA-ES Z-score strategy — 3-input variant (adds uPnL feed).

Architecture: fixed 3→8→3 network, wavelet activations per hidden node.
Inputs (causal, S5 bars):
  f1   = arctan( zscore( (bid_c[i] - bid_c[i-12])  / pip       ) ) / (π/2)
  f10  = arctan( zscore( (bid_c[i] - bid_c[i-600]) / (600*pip) ) ) / (π/2)
  upnl = arctan( pnl_pips/10 - λ×auddp_sum ) / (π/2)  [running score signal, 0 when flat]
...
Usage:
  python3 train_cma_3in.py --pair EUR_JPY --max-hold 200
  python3 train_cma_3in.py --pair EUR_JPY --max-hold 1200 --restarts 30
"""
import argparse
import pickle
import sys
...
W1_END  = N_IN * N_HID          # 24
B1_END  = W1_END + N_HID        # 32
W2_END  = B1_END + N_HID * N_OUT  # 56
B2_END  = W2_END + N_OUT        # 59
ACT_END = B2_END + N_HID        # 67
N_PARAMS = ACT_END
...
def forward(genes, f1, f10, upnl):
    """3→8→3 forward pass. Returns raw output logits (no softmax)."""
    h = np.empty(N_HID)
...
def simulate_chunk(genes, f1, f10, close, pip, spread_pips, max_hold,
    ...
    for i in range(chunk_start, chunk_end):
        f1_i   = f1[i]
        f10_i  = f10[i]
        # running score signal bounded to (-1,+1)
        upnl_i = np.arctan(pnl_pips / 10.0 - LAMBDA_AUDDP * auddp_sum) / half_pi

        out = forward(genes, f1_i, f10_i, upnl_i)
        ...
        if pos == 0 and action != 2:
            ...
            pnl_pips    = -spread_pips  # initial uPnL is -spread
            pos         = direction
...
def main():
    ...
    print(f"CMA-ES Z-score 3-input — AMDDP reward (pnl - λ×auddp, λ={LAMBDA_AUDDP})")
    print(f"Inputs: f1, f10, uPnL  |  Architecture: {N_IN}→{N_HID}→{N_OUT}  params={N_PARAMS}  wavelet activations")
    ...
    lb1_w = 1 if args.gran == "M5" else 12
    lb2_w = 10 if args.gran == "M5" else 600
    compute_zscore_inputs(dummy, 0.01, POP_SIZE, lb1_w, lb2_w)
    dummy_genes = np.zeros(N_PARAMS)
    simulate_chunk(dummy_genes, dummy, dummy, dummy, 0.01, 1.0, max_hold,
                   WARMUP, WARMUP + 40)
    print("  Done\n")
    ...
    tag  = f"cma3in_{args.gran.lower()}_mh{max_hold}_s{args.seed}"
    ...
    if args.oos:
        print("\nOOS evaluation:")
        oos = eval_oos(best_genes, pairs_data, args.max_hold)
