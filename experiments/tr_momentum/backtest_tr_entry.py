"""
TR momentum entry sweep — extended.
...
Run:
    ...
    python3 research/experiments/tr_momentum/backtest_tr_entry.py
"""
...
def run_pair(pair, pip):
    ...
    # ── Walk-forward: all 3 IS chunks positive, ≥5 trades each ──────────
    winners = []
...
for pair, pip in PAIRS:
    ...
    winners, elapsed, oos_days = result

    if not winners:
        print(f"  {pair}: 0 WF winners  ({elapsed:.1f}s)")
        summary.append((pair, 0, 0, 0, "—"))
        continue

    best = winners[0]
    print(f"  {pair}:  {len(winners)} winners  best={best['name']}"
          f"  OOS={best['oos_pd']:+.1f}p/d  WR={best['oos_wr']:.0f}%"
          f"  trades={best['oos_ntrd']}  ({elapsed:.1f}s)")
    summary.append((pair, len(winners), best['oos_pd'], best['oos_wr'], best['name']))

print("\n\n── Summary ─────────────────────────────────────────")
print(f"  {'Pair':<10} {'Winners':>8} {'OOS p/d':>8} {'WR%':>6}  Best config")
print(f"  {'─'*60}")
for pair, nw, opd, owr, cfg in summary:
    flag = "🟢" if opd > 10 else ("🟡" if opd > 0 else "🔴")
    print(f"  {flag} {pair:<10} {nw:>8} {opd:>8.1f} {owr:>6.1f}  {cfg}")
print("\nDone.")
