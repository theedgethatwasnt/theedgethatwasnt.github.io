"""Weighted-sum trigger experiment.

Compute the dashboard's weighted_sum signal at every S5 bar:

  ws = 0.05·S5  + 0.05·M1  + 0.10·5m + 0.15·15m + 0.20·1h + 0.20·4h + 0.25·24h

Where each component is pips/minute over that lookback window.

Entry rule (signal-following):
...
def main():
    ...
    print("="*100)
    print("  Weighted-sum SIGNAL-FOLLOWING trigger (ws>=+thr → long, <=−thr → short)")
    ...
    out_path = OUT / "weighted_sum_trigger.csv"
