...
def sweep_kernel(s5_mom: np.ndarray, s5_close: np.ndarray, s5_spread_pips: np.ndarray,
    ...
    for ci in prange(n_cfg):
        ...
        for i in range(1, n_s5):
            ...
            if in_pos == 0:
                # REVERSED: peak-then-trough → SHORT; trough-then-peak → LONG
...
def main():
    ...
    print("="*100)
    print("  S5 whipsaw STREAMING — REVERSED direction (continuation hypothesis)")
    ...
    out_path = OUT / "s5_whipsaw_reverse.csv"
